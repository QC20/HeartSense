...
ECGJewelEmulator::ECGJewelEmulator(uint8_t pin, uint16_t numPixels)
...
void ECGJewelEmulator::begin() {
    pixels.begin();
    pixels.setBrightness(10);
    pixels.show();
}

void ECGJewelEmulator::update(unsigned long currentMillis, int currentHeartRate) {
...
int ECGJewelEmulator::getECGBrightness(uint16_t step, uint16_t totalSteps) {
...
void ECGJewelEmulator::setLEDs(int brightness) {
...
void ECGJewelEmulator::show() {
    pixels.show();
}

void ECGJewelEmulator::updateHeartRate(int newHeartRate, unsigned long currentMillis) {
...
void ECGJewelEmulator::updateMovingAverage(int newHeartRate) {
...
bool ECGJewelEmulator::isSignificantChange(int newHeartRate) {
...
bool ECGJewelEmulator::isRWavePortion(uint16_t step, uint16_t totalSteps) const {
    float progress = static_cast<float>(step) / totalSteps;
    return progress >= 0.20f && progress < 0.24f;
}

int ECGJewelEmulator::applyIntensityModulation(int brightness) {
